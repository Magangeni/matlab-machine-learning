...
%-------------------------------------------------------------------------------
%   This graphically interactive function trains simple neural nets.
%-------------------------------------------------------------------------------
%   Form:
...
% displayProps(7:10) = {'BackgroundColor',defaultColor,'FontName','Courier'};
% 
...
switch action
  ...
  case 'displayMethod'
    ...
    h = eval( [h.train.method 'NNT( ''show'', h )'] );

  case 'clearMethodDisplay'
    h = eval( [h.train.method 'NNT( ''hide'', h )'] );
  ...
  case 'setUpMethodDisplay'
    ...
    for i = 1:length(w.m)
      if ~strcmp(w.m{i},'Contents.m')
        h = eval( [w.m{i}(1:end-2) '( ''initializeGUI'', h, [left bottom width height] )'] );
        h = eval( [w.m{i}(1:end-2) '( ''hide'', h )'] );
      end
    end      
...
switch action
  ...
  case 'train'
    ...
    %--------------------
      ...
      h = eval( [h.train.method 'NNT( ''train'', h )'] );
...
switch action
  ...
  case 'get test'
    ...
    %---------------------------
    [rows,inCols]  = size( h.train.inputs );
...
switch action
  ...
  case 'initialize'
    ...
    if( exist('NNTrainPreferences.mat','file') )
      load('NNTrainPreferences');

      names = fieldnames( nnTrainPref );
      for j = 1:length(names)
        eval(['h.preferences.' names{j} ' = nnTrainPref.' names{j} ';']);
      end;

      tempPath              = fileparts( which('NNTrainPreferences.mat') );
      h.preferences.file    = fullfile(tempPath,'NNTrainPreferences');

      clear nnTrainPref, tempPath;
    ...
    for i = 1:length(w.m)
      if ~strcmp(w.m{i},'Contents.m')
       ...
       % eval( [w.m{i}(1:end-2) '(''initialize prefs'', h);'] );
      end
    end      
  ...
  case 'setUpMethodDisplay'
    ...
    for i = 1:length(w.m)
      if ~strcmp(w.m{i},'Contents.m')
        h = eval( [w.m{i}(1:end-2) '( ''initialize prefs GUI'', h, [left bottom width height] )'] );
        h = eval( [w.m{i}(1:end-2) '( ''hide prefs'', h )'] );
      end
    end      
  ...
  case 'displayMethod'
    ...
    h = eval( [h.preferences.defaultMethod 'NNT( ''show prefs'', h )'] );
  ...
  case 'save preferences'
    ...
    for j = 1:length(names)
      eval(['nnTrainPref.' names{j} ' = h.preferences.' names{j} ';']);
    end;
...
switch action
  ...
  case 'load'
    ...
    names = fieldnames( eval(['temp.' filename(1:s-1)]) );
    for j = 1:length(names)
      eval(['h.train.' names{j} ' = temp.' filename(1:s-1) '.' names{j} ';']);
    end;
...
switch action
  ...
  case 'save'
    ...
    eval([filename '.inputs = h.train.inputs;'])
    eval([filename '.desOutputs = h.train.desOutputs;'])
    eval([filename '.testSets = h.train.testSets;'])
    eval([filename '.trainSets = h.train.trainSets;'])
    fname = fullfile(pathname, filename);
    eval(['save(''' fname ''',''' filename ''');'])
